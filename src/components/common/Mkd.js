...
const Mkd = ({ markdown }) => {
  ...
  useEffect(() => {
    ...
    // 实现效果很不理想，不知道如何解决
    if(theme === "light") {
      (async () => {await import("highlight.js/styles/stackoverflow-light.css")})();
    } else {
      (async () => {await import("highlight.js/styles/stackoverflow-dark.css")})();
    }
  ...
  return (
    ...
    prose-img:m-auto prose-h2:text-[16px] lg:prose-pre:text-[16px] prose-pre:text-[14px]
